...
For a workflow with a durable call ledger, `prepare_decision(state, questions)` validates and serializes the complete request without network I/O. Its opaque `PreparedDecision` exposes only the encoded byte count, SHA-256 and requested model. Record those fields and reserve the call before `submit_prepared(prepared)` sends it once. A prepared request is bound to its originating client, model and endpoint; neither API silently splits or retries an oversized request.
