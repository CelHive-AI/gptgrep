...
トライグラム grep、必要に応じて使う Jev のルーティング／再ランキング、ローカルの Codex
推論ホスト**を組み合わせます。ベクトルデータベース、検索デーモン、MCP サーバーは使用しません。

GPTgrep は、元文書と照合できる根拠を返します。厳密な検索はローカルで完結し、リモート推論を
行うのは、呼び出し側が `hybrid`、`semantic`、`judge`、`ask`、`summarize` のいずれかを
選択した場合だけです。推論を行うエージェントは、検索を組み合わせ、ツリーを確認し、上限付きで
ノードを読み取ってから回答できます。

これは初期の開発リリースです。[アーキテクチャ](docs/architecture.ja.md)と、
...
ツール実行記録、実際のモデル／推論強度、使用量を分けて報告します。引用の同一性の検査だけで、
